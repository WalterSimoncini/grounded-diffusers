...
for step, file_path in enumerate(tqdm(training_samples)):
    ...
    loss = loss_fn(fusion_segmentation_pred, segmentation)

    optimizer.zero_grad()
    loss.backward()
    optimizer.step()

    torch_writer.add_scalar("train/loss", loss.item(), global_step=step)

    print(f"training step: {step}/{total_steps}, loss: {loss}")
